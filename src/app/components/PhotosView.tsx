...
import { RefreshCw, Images, Trash2, ChevronLeft, ChevronRight, X, Camera } from "lucide-react";
...
export default function PhotosView({ plants }: { plants: PlantItem[] }) {
  ...
  const [viewingPhoto, setViewingPhoto] = useState<GalleryItem | null>(null);
  ...
  const syncLatestPhoto = async (plantId?: string) => {
    if (isCapturing) return; // 手動拍照中不進行自動同步，避免衝突
    ...
    try {
      ...
      const isAlreadyInGallery = items.some(item => item.capturedAtIso === lastUploadTime);
      if (isAlreadyInGallery) return;
      ...
      try {
        ...
        setItems(current => {
          // 再次檢查，防止在非同步過程中重複
          if (current.some(item => item.capturedAtIso === lastUploadTime)) return current;
          return [newItem, ...prune(current)];
        });
      } catch (e) {
    ...
    try {
      ...
      while (captureSeqRef.current === captureSeq && attempts < maxAttempts) {
        ...
        if (isMatched && lastUploadTime) {
          ...
          setItems((prev) => {
            if (prev.some(it => it.capturedAtIso === lastUploadTime)) return prev;
            return [item, ...prune(prev)];
          });
  ...
  const handleConfirmDelete = () => {
    ...
    if (deleteTarget.type === 'all') {
      ...
      if (viewingPhoto?.id === deleteTarget.id) {
    ...
    return (
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden px-6 pt-[calc(env(safe-area-inset-top)+2rem)]">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-2xl font-black text-gray-800 tracking-tight">Photo</h1>
        </div>
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden pt-[env(safe-area-inset-top)]">
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-[24px] p-4 shadow-sm border border-white/60">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-sm font-black text-gray-800">{selectedPlant?.name}</div>
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            {filteredItems.map((it) => (
              <button
                key={it.id}
                onClick={() => setViewingPhoto(it)}
                className="relative rounded-3xl overflow-hidden bg-white shadow-sm border border-gray-100 group active:scale-95 transition-all text-left"
              >
      ...
      {viewingPhoto && (
        <div 
          className="fixed inset-0 z-[110] flex flex-col bg-black/60 backdrop-blur-2xl animate-in fade-in duration-300"
          onClick={() => setViewingPhoto(null)}
        >
          {/* 頂部區域：僅保留居中標題 */}
          <div className="w-full pt-12 pb-4 flex items-center justify-center" onClick={e => e.stopPropagation()}>
            <div className="text-white/40 text-[10px] font-black uppercase tracking-widest">Snapshot Details</div>
          </div>

          {/* 中間彈窗容器：包裹照片、資訊與按鈕，點擊內部不關閉 */}
          <div className="flex-1 flex flex-col items-center justify-center">
            <div 
              className="w-full max-w-[400px] flex flex-col items-center px-4" 
              onClick={e => e.stopPropagation()}
            >
              {/* 照片 */}
              <div className="w-full aspect-[4/3] rounded-[28px] overflow-hidden shadow-2xl ring-1 ring-white/20 animate-in zoom-in-95 duration-300">
                <img src={viewingPhoto.dataUrl} alt="Full view" className="w-full h-full object-cover" />
              </div>
              
              {/* 照片下方資訊 */}
              <div className="mt-5 text-center">
                <div className="text-lg font-black text-white/90">
                  {new Date(viewingPhoto.capturedAtIso).toLocaleTimeString("zh-TW", { hour12: false, hour: '2-digit', minute: '2-digit' })}
                </div>
                <div className="mt-0.5 text-[10px] font-bold text-white/30 uppercase tracking-tighter">
                  {new Date(viewingPhoto.capturedAtIso).toLocaleDateString("zh-TW", { month: 'short', day: 'numeric', year: 'numeric' })}
                </div>
              </div>
              ...
              <div className="w-full mt-8 flex flex-col gap-2">
                <button
                  ...
                  className="w-full py-3.5 rounded-[18px] bg-red-500 text-white text-xs font-black shadow-lg shadow-red-500/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <Trash2 className="w-4 h-4" />
                  <span>刪除照片</span>
                </button>
                <button
                  onClick={() => setViewingPhoto(null)}
                  className="w-full py-3.5 rounded-[18px] bg-white/10 text-white/60 text-xs font-black backdrop-blur-md active:scale-95 transition-all border border-white/5"
                >
                  返回相簿
                </button>
              </div>
